...
from core.domain.missions.state_machine import MissionStateMachine, InvalidMissionTransition

def test_valid_state_transition():
    mission = Mission(title="Test", description="Desc", repository_id="repo1", source="test")
...
def test_terminal_states():
    ...
    for status in terminal_states:
        mission = Mission(title="Test", description="Desc", repository_id="repo", source="src", status=status)
        with pytest.raises(InvalidMissionTransition):
            MissionStateMachine.transition(mission, MissionStatus.CREATED)

def test_cancellation():
    mission = Mission(title="Test", description="Desc", repository_id="repo", source="src", status=MissionStatus.EXECUTING)
...
def test_malformed_state():
    mission = Mission(title="Test", description="Desc", repository_id="repo", source="src")
