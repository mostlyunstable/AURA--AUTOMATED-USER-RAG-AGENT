...
DB_URL = os.environ.get("DATABASE_URL", "postgresql+asyncpg://aura:aura@localhost:5432/aura")
if "sqlite" in DB_URL:
...
async def async_client():
    ...
    async def override_get_mission_service():
        uow = SQLAlchemyUnitOfWork(async_sessionmaker(create_async_engine(DB_URL, poolclass=pool.NullPool), expire_on_commit=False))
...
async def test_execution_approval_flow(async_client):
    payload = {"title": "Approval Test", "description": "Desc", "repository_id": "r1", "source": "test"}
    ...
    resp = await async_client.post(f"/missions/{mission_id}/transition", json={"target_state": "EXECUTING"})
    assert resp.status_code == 409

    resp = await async_client.post(f"/missions/{mission_id}/approve", json={"approval_type": "EXECUTION"})
...
async def test_dag_cycle_detection():
    ...
    t1 = Task(mission_id=mission.id, title="T1", description="D1", task_type=TaskType.ANALYSIS)
    t2 = Task(mission_id=mission.id, title="T2", description="D2", task_type=TaskType.ANALYSIS)
    t3 = Task(mission_id=mission.id, title="T3", description="D3", task_type=TaskType.ANALYSIS)

    await orchestrator.add_task(t1)
    await orchestrator.add_task(t2)
    await orchestrator.add_task(t3)

    await orchestrator.add_dependency(mission.id, TaskDependency(task_id=t2.id, depends_on_task_id=t1.id))
    await orchestrator.add_dependency(mission.id, TaskDependency(task_id=t3.id, depends_on_task_id=t2.id))

    with pytest.raises(ValueError, match="Cycle detected"):
        try:
            await orchestrator.add_dependency(mission.id, TaskDependency(task_id=t1.id, depends_on_task_id=t3.id))
