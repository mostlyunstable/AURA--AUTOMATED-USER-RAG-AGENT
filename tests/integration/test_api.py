...
async def test_create_mission(async_client):
    payload = {"title": "API Test", "description": "Desc", "repository_id": "repo1", "source": "api"}
    response = await async_client.post("/missions", json=payload)
    assert response.status_code == 200

@pytest.mark.asyncio
async def test_mission_flow(async_client):
    payload = {"title": "API Test 2", "description": "Desc 2", "repository_id": "repo2", "source": "api"}
    response = await async_client.post("/missions", json=payload)
    ...
    response = await async_client.post(f"/missions/{mission_id}/transition", json={"target_state": "PLANNING"})
    assert response.status_code == 200
