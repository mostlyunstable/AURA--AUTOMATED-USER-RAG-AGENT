...
from core.domain.execution.enums import EnvironmentStatus, CommandStatus
...
async def temp_git_repo():
    with tempfile.TemporaryDirectory() as td:
        ...
        proc = await asyncio.create_subprocess_exec("git", "commit", "-m", "init", cwd=td)
        await proc.wait()
...
async def test_execution_boundary(async_client, temp_git_repo):
    ...
    payload = {"title": "Execution Test", "description": "Desc", "repository_id": temp_git_repo, "source": "test"}
    ...
    resp = await async_client.post(f"/missions/{mission_id}/tasks", json={
        "title": "Exec Task",
        "description": "Desc",
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("INSERT INTO task_executions (id, task_id, status, attempt_number, started_at, result_metadata) VALUES (:eid, :tid, 'PENDING', 1, :now, '{}')"), {"eid": execution_id, "tid": task_id, "now": datetime.now(timezone.utc)})
        
    resp = await async_client.post(f"/tasks/{task_id}/executions/{execution_id}/environment?mission_id={mission_id}&repository_id={temp_git_repo}")
