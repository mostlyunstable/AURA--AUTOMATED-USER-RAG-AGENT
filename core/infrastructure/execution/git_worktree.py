...
class LocalGitWorktreeManager(GitWorktreeManager):
    ...
    async def create(self, environment: ExecutionEnvironment, repository_id: str) -> str:
        repo_path = os.path.abspath(repository_id)
        worktree_path = os.path.join(self.worktree_root, f"{environment.mission_id}", f"{environment.task_id}", f"{environment.execution_id}")
    ...
    async def remove(self, environment: ExecutionEnvironment) -> None:
        ...
        if not os.path.abspath(environment.worktree_path).startswith(self.worktree_root):
            raise ValueError("Worktree path traversal detected")
