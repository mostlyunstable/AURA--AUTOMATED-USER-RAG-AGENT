...
class ArtifactCollector:
    def __init__(self, worktree_root: str = "~/.aura/worktrees", max_bytes: int = None):
        self.worktree_root = os.path.abspath(os.path.expanduser(worktree_root))
        self.max_bytes = max_bytes or int(os.environ.get("AURA_MAX_ARTIFACT_BYTES", 10 * 1024 * 1024))

    def _resolve_and_validate_path(self, env: ExecutionEnvironment, relative_or_absolute_path: str) -> Optional[str]:
        ...
        if os.path.isabs(relative_or_absolute_path):
            ...
            target_path = os.path.abspath(os.path.join(worktree_abs, relative_or_absolute_path))
    ...
    def collect(self, env: ExecutionEnvironment, file_path: str, artifact_type: ArtifactType) -> Optional[Artifact]:
