...
class SQLAlchemyTaskRepository:
    ...
    async def claim_task(self, task_id: UUID, worker_id: UUID) -> Optional[Task]:
        ...
        model.assigned_agent_id = worker_id
    ...
    async def try_transition_status(
        ...
        result = await self.session.execute(
            ...
            .where(
                TaskModel.id == task_id,
                TaskModel.status == from_status.value,
            )
            .with_for_update()
        )
        model = result.scalar_one_or_none()
...
class SQLAlchemyTaskExecutionRepository:
    ...
    async def get(self, execution_id: UUID) -> Optional[TaskExecution]:
        result = await self.session.execute(
            select(TaskExecutionModel).where(TaskExecutionModel.id == execution_id)
        )
        model = result.scalar_one_or_none()
        if not model:
            return None
        return TaskExecution(
            ...
            status=TaskExecutionStatus(model.status),
    ...
    async def update(self, execution: TaskExecution) -> None:
        model = await self.session.get(TaskExecutionModel, execution.id)
        if model:
            model.agent_id = execution.agent_id
            model.status = execution.status.value
            model.completed_at = execution.completed_at
            model.error = execution.error
            model.result_metadata_ = execution.result_metadata
            await self.session.flush()
...
class SQLAlchemyTaskLeaseRepository(TaskLeaseRepository):
    ...
    async def claim_task(
        ...
        from core.domain.workers.entities import TaskLease
        ...
        result = await self.session.execute(
            ...
            .where(
                ...
                TaskLeaseModel.lease_expires_at < now,
            )
            .with_for_update(skip_locked=True)
        )
        model = result.scalar_one_or_none()
        ...
        model.lease_expires_at = now + timedelta(seconds=lease_ttl_seconds)
        model.last_heartbeat_at = now
        model.renewed_count = 0
        model.updated_at = now
        await self.session.flush()
        return self._to_domain(model)
