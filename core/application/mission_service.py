...
class MissionService:
    ...
    async def create_mission(self, title: str, description: str, repository_id: str, source: str, risk_level: str = "medium") -> Mission:
        mission = Mission(
            title=title,
    ...
    async def update_mission_status(self, mission_id: UUID, new_status: MissionStatus) -> Mission:
        async with self.uow:
    ...
    async def list_missions(self) -> List[Mission]:
        async with self.uow:
            return await self.uow.missions.list()

    async def request_approval(self, mission_id: UUID, approval_type: ApprovalType) -> Approval:
        async with self.uow:
            mission = await self.uow.missions.get(mission_id)
            if not mission:
                raise MissionNotFound(f"Mission {mission_id} not found")
    ...
    async def resolve_approval(self, approval_id: UUID, status: ApprovalStatus, reason: str = None) -> Approval:
        async with self.uow:
            ...
            event_type = "mission.execution_approved" if status == ApprovalStatus.APPROVED else "mission.execution_rejected"
            event = Event(
            ...
            if status == ApprovalStatus.APPROVED and approval.approval_type == ApprovalType.EXECUTION:
                mission = await self.uow.missions.get(approval.mission_id)
                if mission.status == MissionStatus.PLANNED:
                    updated_mission = MissionStateMachine.transition(mission, MissionStatus.APPROVED_FOR_EXECUTION)
                    await self.uow.missions.update(updated_mission)
                    await self.uow.events.append(Event(
                        event_type="mission.state_changed",
                        mission_id=mission.id,
                        metadata={"new_status": MissionStatus.APPROVED_FOR_EXECUTION.value}
                    ))
            elif status == ApprovalStatus.REJECTED:
                mission = await self.uow.missions.get(approval.mission_id)
                if mission.status == MissionStatus.PLANNED:
                    updated_mission = MissionStateMachine.transition(mission, MissionStatus.BLOCKED)
                    await self.uow.missions.update(updated_mission)
                    await self.uow.events.append(Event(
                        event_type="mission.state_changed",
                        mission_id=mission.id,
