...
from core.domain.plans.entities import EngineeringPlan, PlannerOutput, PlanTask
from core.domain.plans.enums import PlanStatus
from core.domain.llm.interfaces import LLMProvider, LLMRequest
from core.domain.context.interfaces import RepositoryContextProvider, ForgeContextProvider, EngineeringContext
from core.domain.dag.validator import DAGValidator, DAGCycleError
...
class PlannerAgent:
    ...
    async def generate_plan(self, mission_id: UUID) -> EngineeringPlan:
        ...
        repo_context = await self.repo_provider.get_context(mission.repository_id, mission)

        forge_context = await self.forge_provider.get_context(mission.repository_id, mission)
            ...
            try:
                ...
                for t in output.tasks:
                    ...
                    for d_title in t.dependencies:
                        ...
                        deps.append(TaskDependency(task_id=t_id, depends_on_task_id=d_id))
                        
                # Validate DAG
                for d in deps:
                    DAGValidator.validate_new_dependency(d, [existing for existing in deps if existing != d])
        ...
        plan = EngineeringPlan(
            ...
            provider="unknown",
            model="unknown",
            output=PlannerOutput(summary=f"Failed to generate valid plan. Last error: {last_error}", assumptions=[], risks=[], tasks=[], validation_strategy={"approach": "none", "tests_required": False}),
