...
class ExecutionService:
    ...
    async def create_environment(self, mission_id: UUID, task_id: UUID, execution_id: UUID, repository_id: str) -> ExecutionEnvironment:
        async with self.uow:
            # Idempotency check
        ...
        async with self.uow:
            ...
            await self.uow.events.append(Event(
                event_type="execution.environment.created" if env.status == EnvironmentStatus.READY else "execution.environment.failed",
                mission_id=mission_id,
                metadata={"environment_id": str(env.id), "status": env.status.value, "failure_reason": env.failure_reason}
            ))
            await self.uow.commit()
            
        return env

    async def execute_command(self, environment_id: UUID, command: ExecutionCommand) -> CommandResult:
        async with self.uow:
    ...
    async def cleanup_environment(self, environment_id: UUID) -> None:
        ...
        async with self.uow:
            await self.uow.execution_environments.update(env)
            await self.uow.events.append(Event(
                event_type="execution.environment.destroyed" if env.status == EnvironmentStatus.DESTROYED else "execution.environment.cleanup_failed",
                mission_id=env.mission_id,
                metadata={"environment_id": str(env.id), "status": env.status.value}
            ))
            await self.uow.commit()
