...
class ExecutionService:
    ...
    async def create_environment(
        ...
        try:
            ...
            aura_execution_environments_created_total.inc()

        except Exception as e:
            env.status = EnvironmentStatus.FAILED
            aura_execution_environments_failed_total.inc()
    ...
    async def cleanup_environment(self, environment_id: UUID) -> None:
        ...
        try:
            ...
            aura_execution_environments_destroyed_total.inc()
        except Exception as e:
            env.status = EnvironmentStatus.FAILED
            aura_execution_environments_failed_total.inc()
            env.failure_reason = f"Cleanup failed: {e}"
            aura_cleanup_failures_total.inc()
