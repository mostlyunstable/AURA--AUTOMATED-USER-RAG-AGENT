...
from core.application.mission_service import MissionService, MissionNotFound
...
from core.domain.tasks.enums import TaskType, TaskStatus
from core.domain.approvals.entities import Approval
from core.domain.approvals.enums import ApprovalType, ApprovalStatus
...
MISSION_TRANSITIONS = Counter("mission_transitions_total", "Total transitions", ["status"])
...
async def create_mission(req: CreateMissionRequest, service: MissionService = Depends(get_mission_service)):
...
async def get_mission(mission_id: UUID, service: MissionService = Depends(get_mission_service)):
...
async def transition_mission(mission_id: UUID, req: TransitionRequest, service: MissionService = Depends(get_mission_service)):
    try:
        ...
        logger.info("mission_transitioned", mission_id=str(mission_id), target_state=req.target_state.value)
        return updated
...
async def list_tasks(mission_id: UUID, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    async with uow:
...
async def create_task(mission_id: UUID, req: CreateTaskRequest, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
...
async def add_dependency(task_id: UUID, req: DependencyRequest, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    orchestrator = MissionOrchestrator(uow)
...
async def request_approve(mission_id: UUID, req: ApproveRequest, service: MissionService = Depends(get_mission_service)):
    approval = await service.request_approval(mission_id, req.approval_type)
    
    # Fast-forward auto approval for testing/simplicity
    approval = await service.resolve_approval(approval.id, ApprovalStatus.APPROVED, "Auto approved")
    APPROVALS_TOTAL.inc()
    return approval
...
async def request_reject(mission_id: UUID, req: RejectRequest, service: MissionService = Depends(get_mission_service)):
    approval = await service.request_approval(mission_id, req.approval_type)
    approval = await service.resolve_approval(approval.id, ApprovalStatus.REJECTED, req.reason)
    return approval


@app.post("/missions/{mission_id}/plan", response_model=EngineeringPlan)
async def generate_plan(mission_id: UUID, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    
    # Use Nvidia if key is present, else Fake
    api_key = os.environ.get("NVIDIA_API_KEY")
    if api_key:
        llm_provider = NvidiaLLMProvider(api_key=api_key)
    else:
...
async def list_plans(mission_id: UUID, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    async with uow:
        return await uow.plans.get_by_mission(mission_id)

@app.get("/plans/{plan_id}", response_model=EngineeringPlan)
async def get_plan(plan_id: UUID, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    async with uow:
...
async def approve_plan(plan_id: UUID, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    async with uow:
        plan = await uow.plans.get(plan_id)
        if not plan:
...
async def reject_plan(plan_id: UUID, service: MissionService = Depends(get_mission_service)):
    uow = service.uow
    async with uow:
        plan = await uow.plans.get(plan_id)
        if not plan:
...
def get_execution_service(service: MissionService = Depends(get_mission_service)) -> ExecutionService:
    uow = service.uow
...
@app.post("/tasks/{task_id}/executions/{execution_id}/environment", response_model=ExecutionEnvironment)
async def create_environment(
    ...
    if not mission or mission.status not in [MissionStatus.APPROVED_FOR_EXECUTION, MissionStatus.EXECUTING]:
        raise HTTPException(status_code=403, detail="Mission is not approved for execution")
        
    return await exec_service.create_environment(mission_id, task_id, execution_id, repository_id)
